...
async function reason(problem, context = {}, options = {}) {
  ...
  const localEndpoint = options.endpoint || process.env.HUGO_LOCAL_AI_ENDPOINT;
  ...
  const body = { model: options.model || process.env.HUGO_LOCAL_AI_MODEL || process.env.HUGO_REASONING_MODEL || 'local-model', temperature: 0.2, messages: [{ role: 'system', content: 'Одговори на македонски. Врати јасно решение, претпоставки и следни дејства.' }, { role: 'user', content: JSON.stringify({ problem, context }) }] };
  try {
    const headers = { 'content-type': 'application/json' };
    if (apiKey) headers.authorization = `Bearer ${apiKey}`;
    const data = await requestJson(endpoint, { method: 'POST', headers, body: JSON.stringify(body) });
    return { provider: localEndpoint && !apiKey ? 'llama.cpp' : 'openai-compatible', content: data.choices?.[0]?.message?.content || '', raw: data };
  } catch (error) {
